      ...
      <div className="space-y-4 sm:space-y-5 lg:space-y-6">
        <MotionWrapper className="rounded-[2.25rem] border border-white/10 bg-white/[0.055] p-4 shadow-glow backdrop-blur-xl sm:p-6 md:p-10">
          ...
          <h1 className="mt-1.5 font-serif text-2xl text-white sm:text-3xl md:text-4xl">
            ค่าตั้งต้นทั้งหมดแก้ไขได้จากไฟล์ config
          </h1>
          <StaggerList className="mt-6 grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <StaggerItem>
              <SettingsCard
...
function SettingsCard({
  ...
  return (
    <div className="h-full rounded-xl border border-white/10 bg-black/10 p-4 transition-colors hover:bg-white/[0.04] dark:bg-black/20 sm:rounded-[1.75rem] sm:p-5">
      <p className="text-[10px] uppercase tracking-[0.22em] text-white/45 sm:text-xs">{title}</p>
      <pre className="mt-2.5 whitespace-pre-wrap font-sans text-xs leading-5 text-white/72 sm:text-sm sm:leading-7">
        {body}
      </pre>
